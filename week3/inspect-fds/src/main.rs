...
fn main() {
    ...
    let ret = ps_utils::get_target(target).expect("problem in calling get_target");
    let _pcs = match ret{
        Some(proc) => {
            proc.print();
            for child in ps_utils::get_child_processes(proc.pid).expect("fail to find children").iter(){
                child.print();
            }
        },
        None  => {
