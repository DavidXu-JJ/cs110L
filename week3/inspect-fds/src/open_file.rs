...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
		...
		let mut path_str=format!("/proc/{}/fd/{}", pid, fd);
        let mut path = std::path::Path::new(&path_str);

        let link = fs::read_link(path).ok()?;
        let ret_name = OpenFile::path_to_name(link.to_str()?);

		path_str=format!("/proc/{}/fdinfo/{}", pid, fd);
        path = std::path::Path::new(&path_str);

        let fdinfo = fs::read_to_string(path).ok()?;
        let ret_cursor = OpenFile::parse_cursor(&fdinfo)?;
        let ret_access_mode = OpenFile::parse_access_mode(&fdinfo)?;

		let ret=OpenFile::new(ret_name, ret_cursor, ret_access_mode);
        Some(ret)
    }
