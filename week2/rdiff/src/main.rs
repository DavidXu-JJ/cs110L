...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..seq1.len(){
        for j in 0..seq2.len(){
            if seq1[i] == seq2[j]{
                dp.set(i + 1, j + 1, dp.get(i + 1, j + 1) .unwrap()
                    .max(dp.get(i, j).unwrap() + 1)); 
            }else{
                dp.set(i + 1, j + 1, dp.get(i + 1, j + 1) .unwrap()
                    .max(dp.get(i, j + 1).unwrap())); 
                dp.set(i + 1, j + 1, dp.get(i + 1, j + 1) .unwrap()
                    .max(dp.get(i + 1, j).unwrap())); 
            }
        }
    }
    dp
...
fn main() {
    ...
    let lcs = lcs(&file1, &file2); 
    print_diff(&lcs, &file1, &file2, file1.len(), file2.len()); 
    // Be sure to delete the #[allow(unused)] line above
}
